...
# METHOD - PRINT OFF CORRELATION COEFFICIENTS FOR K-MEANS EXTREMES OF GENE EXPRESSION
# Will need to resolve issues with combSuspectedList if this method is used again - is deprecated for now
getSuspectedCoeffValues <- function(pathName, eyeOrExpress) {
  # Needed variables
  ...
  for(i in names(filteredGeneList)) {
    for(j in combSuspectedList) {
      if(j == i) {
        ...
        pValue <- cor.test(fetchedGene, sortedAverageEyeSizes, method = "pearson")
        ...
        correlationCoefficients[inc] <- cor(fetchedGene, sortedAverageEyeSizes, method = "pearson")
        ...
        print(ggplot(tempDataFrame, aes(x=sortedAverageEyeSizes, y=fetchedGene, label=sortedRh1ClusterData$Strain, color=Clusters)) + 
                labs(x="Average Eye Size (Pixels)", y="Average Genetic Experssion", title=paste("Gene", names(filteredGeneList[i]), ", P-value:", pValue)) +
                geom_point() + geom_text(aes(label=sortedRh1ClusterData$Strain), hjust=0, vjust=0,size=2))
        ggsave(paste(pathName, "SuspectGene", inc, ".jpeg", sep = ""))
  ...
  print(ggplot(tempDataFrame, aes(x = coeffNames, y = coeff, label = pList, color=pValues)) + geom_point() + 
    labs(title="Correlation Coefficients for Suspected Genes", x="Genes", y="Coefficient Value") +
  ...
  ggsave(paste(pathName, "CoefficientPlotB.jpeg", sep = ""))
}
